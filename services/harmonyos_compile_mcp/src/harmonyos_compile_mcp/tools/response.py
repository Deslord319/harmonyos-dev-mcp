"""Unified MCP response envelope for harmonyos_compile_mcp tools."""
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    logger.exception(f"Tool {tool} unexpected error: {exc}")
...
def _to_mcp_result(envelope: Dict[str, Any]) -> Dict[str, Any]:
    text = f'{envelope["tool"]}: ok' if envelope["ok"] else f'{envelope["tool"]}: {(envelope.get("error") or {}).get("detail") or "Operation failed"}'
    return {
...
def _normalize_result(tool: str, raw: Any, request_id: str, duration_ms: int) -> Dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        return _ok_envelope(tool=tool, result=raw, request_id=request_id, duration_ms=duration_ms)
...
def error_result(code: str, detail: str, *, result: Any = None, tool: str = "") -> Dict[str, Any]:
    return {
        "tool": tool,
        "ok": False,
        "result": result,
    ...
    *,
    ...
    if isinstance(merged, dict):
        for key, value in raw.items():
            if key not in {"success", "error", "error_code", "hint", "retryable"}:
                merged[key] = value
...
def _error_envelope(tool: str, code: str, detail: str, request_id: str, duration_ms: int) -> Dict[str, Any]:
    return {
        "tool": tool,
        "ok": False,
        "result": None,
